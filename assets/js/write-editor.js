...
async function aiWriteChapter(){
  ...
  if(result){
    ...
    // ===== v53: 合并自修+钩子增强为单次API调用（减少调用次数）=====
    var needFix = _qReport && _qReport.score < 75;
    var needHook = _qReport && _qReport.weaknesses && _qReport.weaknesses.join(',').indexOf('钩子') >= 0;
    if ((needFix || needHook) && typeof callRealAPIWithFallback === 'function') {
      statusBar.style.background = '#fef3c7';
      statusBar.style.color = '#92400e';
      var fixLabel = needFix ? ('质量分 ' + _qReport.score + '/100') : '';
      var hookLabel = needHook ? '钩子不足' : '';
      statusBar.textContent = '🔧 质量增强中…' + (fixLabel ? ' ' + fixLabel : '') + (hookLabel ? ' ' + hookLabel : '');
      
      var combinedPrompt = '你是一位资深网文编辑。请对以下章节进行一次综合优化，同时完成两项任务：\n\n';
      combinedPrompt += '【任务1：质量修复】\n';
      if (needFix) {
        combinedPrompt += '当前质量分：' + _qReport.score + '/100\n';
        if (_qReport.weaknesses && _qReport.weaknesses.length) {
          combinedPrompt += '必须修复的短板：\n';
          _qReport.weaknesses.forEach(function(w, i) { combinedPrompt += (i+1) + '. ' + w + '\n'; });
        }
        combinedPrompt += '修复原则：\n';
        combinedPrompt += '- 缺少对话：增加2-3轮人物对话\n- 开篇偏平：重写开头200字\n- 模板化表达：替换AI腔\n- 信息密度低：删冗余描写\n- 段落过长：拆分超300字段落\n';
        combinedPrompt += '- 保持原有剧情走向、角色性格、对话风格不变\n';
      } else {
        combinedPrompt += '无需修复，保持原文不变。\n';
      }
      combinedPrompt += '\n【任务2：章末钩子增强】\n';
      if (needHook) {
        combinedPrompt += '请重写最后200-300字，使用以下钩子策略之一：\n';
        combinedPrompt += '- 中断动作型：在动作即将发生的瞬间突然中断\n';
        combinedPrompt += '- 信息揭露型：揭露一个信息，同时制造更大的疑问\n';
        combinedPrompt += '- 情绪悬置型：情绪悬而未决，给读者留下强烈余韵\n';
      } else {
        combinedPrompt += '无需修改结尾。\n';
      }
      combinedPrompt += '\n【输出】直接输出修改后的完整正文，不要加任何解释或标记。\n\n';
      combinedPrompt += '【原文】\n' + result;
      
      try {
        var fixResult = await callRealAPIWithFallback(combinedPrompt, null, 'quality_boost', Math.max(500, result.length));
        if (fixResult && fixResult.length > 100 && fixResult !== result) {
          ...
          statusBar.style.background = '#dcfce7';
          statusBar.style.color = '#166534';
          statusBar.textContent = '🔧 质量增强完成 → 质量分 ' + (_qReport ? _qReport.score + '/100' : 'N/A');
        }
      } catch(fixErr) { console.warn('[quality-boost] 失败:', fixErr); }
    }
