...
var DB = {
  ...
  apiKeys: {},
  // { dashscope: ['key1','key2'], deepseek: ['key1'] }
  settings: {
    dark: false,
    fontSize: 15,
    theme: 'blue',
  ...
  // === v44+:安全JSON解析（解析失败返回null）===
  _tryParseJSON: function _tryParseJSON(s) {
    if (!s) return null;
    try { return JSON.parse(s); } catch (e) { return null; }
  },
  // === v44+:从拆分存储 key 反推重建 works（主key丢失后兜底）===
  _rebuildWorksFromSplitKeys: function _rebuildWorksFromSplitKeys() {
    var self = this;
    var prefix = self._SPLIT_KEY_PREFIX; // 'wxbj_w_'
    var seen = {};
    // 扫描 localStorage 中所有 wxbj_w_ 前缀 key
    try {
      for (var i = 0; i < localStorage.length; i++) {
        var k = localStorage.key(i);
        if (!k || k.indexOf(prefix) !== 0) continue;
        // key 格式: wxbj_w_<id> 或 wxbj_w_<id>_<subkey>
        var rest = k.substring(prefix.length);
        // 去掉 _<subkey> 后缀，拿到 id
        var idMatch = rest.match(/^([0-9a-fA-F\-]+?)(?:_|$)/);
        if (!idMatch) continue;
        var wid = idMatch[1];
        if (!wid || wid === 'v4') continue; // 排除主key wxbj_w_v4 之类
        seen[wid] = true;
      }
    } catch (e) {}
    ...
    ids.forEach(function (id) {
      try {
        // 先尝试读取作品元数据 (wxbj_w_<id>)
        var metaStr = localStorage.getItem(prefix + id);
        var meta = metaStr ? self._tryParseJSON(metaStr) : null;
        var w = meta && typeof meta === 'object' ? meta : { id: id, title: '恢复的作品 #' + id.slice(-6) };
        if (!w.id) w.id = id;
        if (!w.title) w.title = '恢复的作品 #' + id.slice(-6);
        // 合并子字段
        w = self._stitchHeavyWork(w);
        // 确保基础字段
        if (!w.chapters) w.chapters = [];
        if (!w.created) w.created = Date.now();
        works.push(w);
      } catch (e) {
        console.warn('[Rebuild] 作品 ' + id + ' 重建失败:', e);
      }
    });
    works.sort(function (a, b) { return (b.created || 0) - (a.created || 0); });
    return works;
  },
  // 初始化（带多源数据恢复：主key → 备份key → IndexedDB 镜像 → 拆分key 反推）
  init: function init() {
    var self = this;
    try {
      var MAIN_KEY = 'wxbj_data_v4';
      var BACKUP_KEY = 'wxbj_data_v4_backup';
      var data = null;
      var recoverySource = null;

      // Step 1: 读取主 key
      var saved = localStorage.getItem(MAIN_KEY);
      data = saved ? self._tryParseJSON(saved) : null;

      // Step 2: 主 key 失败 → 尝试备份 key
      if (!data) {
        var backupStr = localStorage.getItem(BACKUP_KEY);
        if (backupStr) {
          var backupData = self._tryParseJSON(backupStr);
          if (backupData && backupData.works && backupData.works.length > 0) {
            data = backupData;
            recoverySource = 'backup';
            console.warn('[DB] 主key丢失，从备份key恢复数据');
          }
        }
      }

      // Step 3: 仍失败 → 尝试从 IndexedDB AppMemory 镜像加载（异步完成后会再次刷新UI）
      var needsIndexedDBRecovery = !data;

      // Step 4: 仍失败 → 从拆分存储 key 反推作品
      var needsSplitKeyRebuild = !data;
      ...
      if (data) {
        ...
        // 写回主key（防止下次启动再次失败）
        if (recoverySource === 'backup') {
          try { localStorage.setItem(MAIN_KEY, backupStr); } catch (e) {}
        }
      } else if (needsSplitKeyRebuild) {
        // 从拆分存储重建
        var rebuilt = self._rebuildWorksFromSplitKeys();
        if (rebuilt && rebuilt.length > 0) {
          this.works = rebuilt;
          recoverySource = 'splitkeys';
          console.warn('[DB] 主key/备份key都丢失，从拆分存储key反推恢复 ' + rebuilt.length + ' 个作品');
          // 立即写回主 key 和备份 key
          try { self.save(); } catch (e) {}
        }
      }

      // Step 3（接续）：异步尝试 IndexedDB 镜像加载
      if (needsIndexedDBRecovery && typeof self.loadAppMemoryMirror === 'function') {
        try {
          self.loadAppMemoryMirror().then(function (mirror) {
            if (!mirror || !mirror.jsonText) return;
            var mirrorData = self._tryParseJSON(mirror.jsonText);
            if (!mirrorData || !mirrorData.works || mirrorData.works.length <= (self.works || []).length) return;
            console.warn('[DB] IndexedDB 镜像恢复更完整数据，已刷新');
            self.works = mirrorData.works || self.works;
            self.apiConfig = mirrorData.apiConfig || self.apiConfig;
            self.apiKeys = mirrorData.apiKeys || self.apiKeys;
            self.settings = mirrorData.settings || self.settings;
            self._trash = Array.isArray(mirrorData._trash) ? mirrorData._trash : [];
            if (mirrorData._split === true) {
              self.works = self.works.map(function (w) { return self._stitchHeavyWork(w); });
            }
            // 写回 localStorage 主key
            try { self.save(); } catch (e) {}
            // 如果有 onDBReady 回调，触发
            if (typeof window.onDBRecovered === 'function') window.onDBRecovered();
          }).catch(function () {});
        } catch (e) {}
      }
      // 数据迁移：确保所有作品都有 longMemory 和 settings
      this.works.forEach(function (w) {
        if (!w.longMemory) w.longMemory = {
          charStates: [],
          plotThreads: [],
          foreshadows: [],
          charArcs: [],
        ...
        if (!w.longMemory.memoryAnchors) w.longMemory.memoryAnchors = {
          core: [],
          characterTags: [],
          relationships: [],
          items: [],
        ...
        if (!w.longMemory.lifecycle) w.longMemory.lifecycle = {
          lastCompressedAt: -1,
          lastRebuildAt: 0
        };
  ...
  // ========== v40：多作品防串隔离 ==========
  ensureWorkFingerprint: function ensureWorkFingerprint(work) {
    if (!work) return '';
  ...
  ensureAllFingerprints: function ensureAllFingerprints() {
    var self = this;
    (self.works || []).forEach(function (w) {
      self.ensureWorkFingerprint(w);
    });
  },
  validateWorkIsolation: function validateWorkIsolation(work, silent) {
    if (!work || !work.id) return false;
    this.ensureWorkFingerprint(work);
    var currentId = '';
    try {
      currentId = localStorage.getItem('last_edit_work') || '';
    } catch (e) {}
  ...
  openAppMemoryDB: function openAppMemoryDB() {
    var self = this;
    return new Promise(function (resolve, reject) {
      ...
      req.onupgradeneeded = function (e) {
        ...
        if (!db.objectStoreNames.contains(self._appMemoryStore)) {
          db.createObjectStore(self._appMemoryStore, {
            keyPath: 'id'
          });
        }
        if (!db.objectStoreNames.contains(self._chapterStore)) {
          var chStore = db.createObjectStore(self._chapterStore, {
            keyPath: 'id'
          });
          try {
            chStore.createIndex('workId', 'workId', {
              unique: false
            });
          } catch (e) {}
        }
        if (!db.objectStoreNames.contains(self._backupStore)) {
          var bkStore = db.createObjectStore(self._backupStore, {
            keyPath: 'id'
          });
          try {
            bkStore.createIndex('createdAt', 'createdAt', {
              unique: false
            });
          } catch (e) {}
        }
        if (!db.objectStoreNames.contains(self._historyStore)) {
          var hs = db.createObjectStore(self._historyStore, {
            keyPath: 'id'
          });
          try {
            hs.createIndex('chapterKey', 'chapterKey', {
              unique: false
            });
          } catch (e) {}
          try {
  ...
  saveAppMemoryMirror: function saveAppMemoryMirror(jsonText) {
    var self = this;
    try {
      ...
      return self.openAppMemoryDB().then(function (db) {
        return new Promise(function (resolve, reject) {
          ...
          tx.onerror = function () {
            reject(tx.error || new Error('App记忆镜像保存失败'));
          };
        });
      }).catch(function (e) {
  ...
  loadAppMemoryMirror: function loadAppMemoryMirror() {
    var self = this;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        ...
        req.onerror = function () {
          reject(req.error || new Error('读取App记忆镜像失败'));
        };
      });
    });
  },
  ...
  markChapterShardDirty: function markChapterShardDirty(workId, chapterIdx) {
    if (!workId && workId !== 0) return;
    if (!this._dirtyChapterShards) this._dirtyChapterShards = {};
    this._dirtyChapterShards[workId + '::' + chapterIdx] = {
      workId: workId,
  ...
  flushDirtyChapterShards: function flushDirtyChapterShards() {
    var self = this;
    ...
    keys.forEach(function (key) {
      var d = dirty[key];
      p = p.then(function () {
        var w = (self.works || []).find(function (x) {
          return x.id === d.workId;
        });
  ...
  startAutoMaintenance: function startAutoMaintenance() {
    var self = this;
  ...
  getAutoMaintenanceStatus: function getAutoMaintenanceStatus() {
  ...
  saveChapterShard: function saveChapterShard(workId, chapterIdx, chapter) {
    var self = this;
    if (!workId || !chapter) return Promise.resolve(false);
    var _work = (self.works || []).find(function (w) {
      return w.id === workId;
    });
    ...
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        ...
        tx.onerror = function () {
          reject(tx.error || new Error('章节分片保存失败'));
        };
      });
    }).catch(function (e) {
  ...
  saveWorkChapterShards: function saveWorkChapterShards(work) {
    var self = this;
    ...
    chapters.forEach(function (ch, idx) {
      p = p.then(function () {
        return self.saveChapterShard(work.id, idx, ch || {});
      });
    });
    return p.then(function () {
      return true;
  ...
  saveAllChapterShards: function saveAllChapterShards() {
    var self = this;
    var works = self.works || [];
    var p = Promise.resolve(true);
    works.forEach(function (w) {
      p = p.then(function () {
        return self.saveWorkChapterShards(w);
      });
    });
    return p.then(function () {
      return true;
  ...
  getChapterShardCount: function getChapterShardCount() {
    var self = this;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        ...
        req.onerror = function () {
          reject(req.error || new Error('统计章节分片失败'));
        };
      });
    }).catch(function () {
      return 0;
  ...
  // ========== v41：自动备份系统 ==========
  buildBackupPayload: function buildBackupPayload(includeApiKeys) {
    try {
      this.ensureAllFingerprints();
    } catch (e) {}
    return {
  ...
  safeBackupFileName: function safeBackupFileName() {
    var d = new Date();
  ...
  saveInternalBackup: function saveInternalBackup(payload) {
    var self = this;
    ...
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        ...
        tx.onerror = function () {
          reject(tx.error || new Error('内部备份保存失败'));
        };
      });
    }).then(function (row) {
  ...
  listInternalBackups: function listInternalBackups() {
    var self = this;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        ...
        req.onerror = function () {
          reject(req.error || new Error('读取备份清单失败'));
        };
      });
    }).catch(function () {
      return [];
  ...
  trimInternalBackups: function trimInternalBackups(maxCount) {
    var self = this;
    ...
    self.listInternalBackups().then(function (rows) {
      ...
      self.openAppMemoryDB().then(function (db) {
        ...
        rows.slice(maxCount).forEach(function (r) {
          st.delete(r.id);
        });
      });
    });
  },
  saveBackupDirectoryHandle: function saveBackupDirectoryHandle(handle) {
    var self = this;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        var tx = db.transaction([self._appMemoryStore], 'readwrite');
        tx.objectStore(self._appMemoryStore).put({
          id: '_backup_dir_handle',
          handle: handle,
          savedAt: Date.now()
        });
        tx.oncomplete = function () {
        ...
        tx.onerror = function () {
          reject(tx.error || new Error('备份文件夹保存失败'));
        };
      });
    });
  },
  loadBackupDirectoryHandle: function loadBackupDirectoryHandle() {
    var self = this;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        var tx = db.transaction([self._appMemoryStore], 'readonly');
        ...
        req.onsuccess = function () {
          resolve(req.result && req.result.handle ? req.result.handle : null);
        };
        req.onerror = function () {
          reject(req.error || new Error('读取备份文件夹失败'));
        };
      });
    }).catch(function () {
      return null;
  ...
  chooseAutoBackupDirectory: function chooseAutoBackupDirectory() {
    var self = this;
    if (!window.showDirectoryPicker) return Promise.reject(new Error('当前环境不支持选择自动备份文件夹'));
    return window.showDirectoryPicker({
      mode: 'readwrite'
    }).then(function (handle) {
      return self.saveBackupDirectoryHandle(handle).then(function () {
        return handle;
      });
    });
  },
  writeBackupFile: function writeBackupFile(payload, fileName) {
    var self = this;
    ...
    return self.loadBackupDirectoryHandle().then(function (handle) {
      ...
      var verify = handle.queryPermission ? handle.queryPermission({
        mode: 'readwrite'
      }) : Promise.resolve('granted');
      return verify.then(function (state) {
        if (state !== 'granted' && handle.requestPermission) return handle.requestPermission({
          mode: 'readwrite'
        });
        return state;
      }).then(function (state) {
  ...
  scheduleAutoBackup: function scheduleAutoBackup(force) {
    var self = this;
    if (self._autoBackupTimer) clearTimeout(self._autoBackupTimer);
    self._autoBackupTimer = setTimeout(function () {
      self.performAutoBackup(!!force);
    }, force ? 200 : 6000);
  },
  performAutoBackup: function performAutoBackup(force) {
    var self = this;
    var now = Date.now();
  ...
  loadChapterShard: function loadChapterShard(workId, chapterIdx) {
    var self = this;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        var tx = db.transaction([self._chapterStore], 'readonly');
        ...
        req.onerror = function () {
          reject(req.error || new Error('读取章节分片失败'));
        };
      });
    }).catch(function () {
      return null;
  ...
  listChapterHistory: function listChapterHistory(workId, chapterIdx) {
    var self = this,
      key = workId + '::' + chapterIdx;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        ...
        req.onerror = function () {
          reject(req.error || new Error('读取章节历史失败'));
        };
      });
    }).catch(function () {
      return [];
  ...
  restoreChapterHistory: function restoreChapterHistory(historyId) {
    var self = this;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        var tx = db.transaction([self._historyStore], 'readonly');
        ...
        req.onerror = function () {
          reject(req.error || new Error('读取历史版本失败'));
        };
      });
    }).then(function (row) {
      if (!row) throw new Error('历史版本不存在');
      var w = (self.works || []).find(function (x) {
        return x.id === row.workId;
      });
      if (!w) throw new Error('作品不存在');
      if (!w.chapters) w.chapters = [];
      if (!w.chapters[row.chapterIdx]) w.chapters[row.chapterIdx] = {
        title: row.title || '第' + (row.chapterIdx + 1) + '章',
        content: ''
      };
  ...
  safeSlimCurrentWork: function safeSlimCurrentWork(keepRecent) {
    var self = this,
      w = self.getWork();
    keepRecent = keepRecent || 5;
    if (!w) return Promise.reject(new Error('请先选择作品'));
    if (!Array.isArray(w.chapters)) return Promise.resolve({
      slimmed: 0
    });
  ...
  exportCurrentWorkBackup: function exportCurrentWorkBackup() {
    var w = this.getWork();
    if (!w) {
      if (window.showToast) window.showToast('请先选择作品');
      return;
    }
    var payload = {
      _version: 44,
      backupType: 'single-work',
      exportedAt: new Date().toISOString(),
      work: w
    };
  ...
  importSingleWorkBackup: function importSingleWorkBackup(jsonStr) {
  ...
  buildForeshadowBoard: function buildForeshadowBoard() {
    var out = [];
    (this.works || []).forEach(function (w) {
      var lm = w.longMemory || {};
      (lm.foreshadowLedger || []).forEach(function (f) {
        out.push({
          work: w.title || '未命名',
          text: f.text || f.line || '',
          status: f.status || '未解',
          chapter: f.chapterIdx
        });
      });
      (lm.memoryAnchors && lm.memoryAnchors.hooks || []).forEach(function (f) {
        out.push({
          work: w.title || '未命名',
          text: f.text || '',
          status: f.status || '未解',
  ...
  buildCharacterBoard: function buildCharacterBoard() {
    var out = [];
    (this.works || []).forEach(function (w) {
      var lm = w.longMemory || {};
      Object.keys(lm.characterProfiles || {}).forEach(function (name) {
        ...
        out.push({
          ...
          location: p.location || '',
          relationCount: (p.relationships || []).length
        });
      });
      (lm.charStates || []).forEach(function (c) {
        out.push({
          work: w.title || '未命名',
          name: c.name,
          status: c.status || '',
  ...
  buildTimelineBoard: function buildTimelineBoard() {
    var out = [];
    (this.works || []).forEach(function (w) {
      var lm = w.longMemory || {};
      (lm.timelineEvents || []).forEach(function (t) {
        out.push({
          work: w.title || '未命名',
          chapter: t.chapterIdx,
          time: t.time || '',
          text: t.text || ''
        });
      });
      (lm.chapterIndex || []).forEach(function (t) {
        if (t.summary) out.push({
          work: w.title || '未命名',
          chapter: t.chapterIdx,
          time: '',
          text: t.summary
  ...
  getChapterHistoryCount: function getChapterHistoryCount() {
    var self = this;
    return self.openAppMemoryDB().then(function (db) {
      return new Promise(function (resolve, reject) {
        var tx = db.transaction([self._historyStore], 'readonly');
        ...
        req.onerror = function () {
          reject(req.error || new Error('统计章节历史失败'));
        };
      });
    }).catch(function () {
      return 0;
  ...
  buildSlimHealthReport: function buildSlimHealthReport() {
    ...
    works.forEach(function (w) {
      ...
      if (!biggest || wc > biggest.words) biggest = {
        title: w.title || '未命名',
        words: wc,
        id: w.id
      };
  ...
  buildGlobalHealthReport: function buildGlobalHealthReport() {
    var works = this.works || [];
    var report = {
      works: works.length,
      chapters: 0,
  ...
  searchAllWorks: function searchAllWorks(keyword) {
    ...
    (this.works || []).forEach(function (w) {
      ['world', 'chars', 'outline', 'detail'].forEach(function (k) {
        ...
        if (idx >= 0) out.push({
          ...
          type: k,
          chapter: '',
          text: txt.slice(Math.max(0, idx - 40), idx + 80)
        });
      });
      (w.chapters || []).forEach(function (ch, i) {
        ...
        if (idx >= 0) out.push({
          work: w.title || '未命名',
          type: '正文',
          chapter: ch.title || '第' + (i + 1) + '章',
          text: txt.slice(Math.max(0, idx - 40), idx + 100)
        });
      });
    });
    return out.slice(0, 80);
  },
  exportCurrentWorkTxt: function exportCurrentWorkTxt() {
    var w = this.getWork();
    if (!w) {
      if (window.showToast) window.showToast('请先选择作品');
      return;
  ...
  restoreFromAppMemoryMirror: function restoreFromAppMemoryMirror() {
    var self = this;
  ...
  getAppMemoryReport: function getAppMemoryReport() {
    var self = this;
    var localSize = 0;
    try {
    ...
    return Promise.all([self.loadAppMemoryMirror().catch(function () {
      return null;
    }), self.getChapterShardCount ? self.getChapterShardCount() : Promise.resolve(0)]).then(function (res) {
      var mirror = res[0],
        shardCount = res[1] || 0;
      return {
        localSize: localSize,
        ...
        autoMaintenance: self.getAutoMaintenanceStatus ? self.getAutoMaintenanceStatus() : {
          enabled: false,
          pending: 0,
          lastFlushAt: 0
        }
  ...
  // 检查是否有任何API Key
  hasAnyKey: function hasAnyKey() {
    return Object.values(this.apiKeys).some(function (k) {
      return k && k.length > 0;
    });
  },
  // 立即保存（返回 true/false，失败时提示用户）
  saveImmediate: function saveImmediate(notifyFail) {
    try {
      ...
      if (json.length > 2 * 1024 * 1024) {
        ...
        try {
          this._autoSplitWorks();
        } catch (splitErr) {
          console.warn('[Split] 拆分失败:', splitErr);
        }
        // 重算轻量化后的JSON
        json = JSON.stringify({
          _version: 44,
      ...
      // v44+：写备份 key（防止主 key 损坏或被浏览器清理）
      try { localStorage.setItem('wxbj_data_v4_backup', json); } catch (e) {}
      // v36/v37：App级大容量镜像 + 章节正文分片，异步写入，不阻塞页面
      try {
        this.saveAppMemoryMirror(json);
      } catch (mirrorErr) {}
      try {
        this.saveAllChapterShards();
      } catch (shardErr) {}
      try {
        this.scheduleAutoBackup(false);
      } catch (backupErr) {}
      return true;
    } catch (e) {
  ...
  _autoSplitWorks: function _autoSplitWorks() {
    var self = this;
    var works = self.works || [];
    var keysToClean = [];
    // 收集当前已有的拆分key
  ...
  _makeLightWorks: function _makeLightWorks() {
    var works = this.works || [];
  ...
  _stitchHeavyWork: function _stitchHeavyWork(lightWork) {
    ...
    try {
      ...
      if (!lm.memoryAnchors) lm.memoryAnchors = {
        core: [],
        characterTags: [],
        relationships: [],
        items: [],
      ...
      if (!lm.lifecycle) lm.lifecycle = {
        lastCompressedAt: -1,
        lastRebuildAt: 0
      };
      if (!lightWork.archStatus) lightWork.archStatus = {
        world: 'pending',
        chars: 'locked',
        outline: 'locked',
        detail: 'locked'
  ...
  _compressLongMemoryForStorage: function _compressLongMemoryForStorage(lm, aggressive) {
    if (!lm || _typeof(lm) !== 'object') return lm;
    ...
    if (lm.factionGraph) {
      ...
      if (fNames.length > 30) {
        ...
        fNames.slice(20).forEach(function (n) {
          delete lm.factionGraph[n];
        });
      }
    ...
    if (lm.itemLedger) {
      ...
      if (iNames.length > 40) {
        ...
        iNames.slice(25).forEach(function (n) {
          delete lm.itemLedger[n];
        });
      }
    ...
    if (lm.rollingSummary) {
      if (_typeof(lm.rollingSummary) === 'object') {
    ...
    if (lm.characterHistory) {
      ...
      if (chNames.length > 60) {
        ...
        chNames.slice(40).forEach(function (n) {
          delete lm.characterHistory[n];
        });
      }
  ...
  listTrash: function listTrash() {
    this._trash = this._trash || [];
    var now = Date.now();
    // 自动清理 30 天以上
    this._trash = this._trash.filter(function (t) {
      return now - (t.deletedAt || 0) < 30 * 24 * 3600 * 1000;
    });
    return this._trash;
  },
  // 从回收站恢复
  restoreFromTrash: function restoreFromTrash(id) {
    this._trash = this._trash || [];
    var idx = this._trash.findIndex(function (t) {
      return t.work && t.work.id === id;
    });
    if (idx < 0) return false;
    var w = this._trash[idx].work;
    this._trash.splice(idx, 1);
    if (!this.works.find(function (x) {
      return x.id === w.id;
    })) this.works.push(w);
    this.flush();
    return true;
  ...
  exportAll: function exportAll(silent) {
    var exportData = {
      _version: 44,
      exportedAt: new Date().toISOString(),
      works: this.works,
  ...
  importFromJSON: function importFromJSON(jsonStr) {
    try {
      if (!jsonStr || jsonStr.length > 50 * 1024 * 1024) {
        throw new Error('文件过大或为空（限制 50MB）');
      }
      var data = JSON.parse(jsonStr);
      if (!data || _typeof(data) !== 'object') throw new Error('文件不是有效 JSON');
      ...
      for (var i = 0; i < data.works.length; i++) {
        var w = data.works[i];
        // 字段合法性
        if (!w || _typeof(w) !== 'object' || !w.id || typeof w.id !== 'string') {
          rejected++;
          continue;
        }
        if (w.id.length > 100) {
        ...
        delete w.prototype;
        var exists = this.works.find(function (x) {
          return x.id === w.id;
        });
        if (exists) {
          skipped++;
  ...
  getWork: function getWork() {
    var id = localStorage.getItem('last_edit_work');
    if (!id || !this.works) return null;
    return this.works.find(function (w) {
      return w.id === id;
    }) || this.works[0];
  },
  // 保存作品
  saveWork: function saveWork(work) {
    if (!this.validateWorkIsolation(work)) return false;
    this.ensureWorkFingerprint(work);
    var idx = this.works.findIndex(function (w) {
      return w.id === work.id;
    });
    if (idx >= 0) {
      this.works[idx] = work;
    ...
    try {
      this.markWorkShardsDirty(work);
    } catch (shardErr) {}
    this.save();
    return true;
  },
  // 创建新作品
  createWork: function createWork(title, desc, category) {
    // 确保ID绝对唯一（防止同一毫秒创建多个作品时冲突）
    var id;
    do {
      id = 'w_' + Date.now() + '_' + Math.floor(Math.random() * 1000).toString().padStart(3, '0');
    } while (this.works.some(function (w) {
      return w.id === id;
    }));
    var work = {
      id: id,
  ...
  deleteWork: function deleteWork(id) {
    var target = this.works.find(function (w) {
      return w.id === id;
    });
    this.works = this.works.filter(function (w) {
      return w.id !== id;
    });
    if (target) {
      this._trash = this._trash || [];
  ...
  // 物理删除（不入回收站，用于回收站清空）
  hardDeleteWork: function hardDeleteWork(id) {
    this.works = this.works.filter(function (w) {
      return w.id !== id;
    });
    this.flush();
  ...
  // 保存API配置
  saveApiConfig: function saveApiConfig(config) {
    this.apiConfig = config;
    this.save();
  },
  // 获取某个服务商的所有密钥
  getApiKeys: function getApiKeys(provider) {
    return this.apiKeys[provider] || [];
  },
  // 添加密钥
  addApiKey: function addApiKey(provider, key) {
    if (!this.apiKeys[provider]) this.apiKeys[provider] = [];
    if (!this.apiKeys[provider].includes(key)) {
      this.apiKeys[provider].push(key);
      this.save();
    }
  },
  // 删除密钥
  removeApiKey: function removeApiKey(provider, index) {
