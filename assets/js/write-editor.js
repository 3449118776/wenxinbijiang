...
function buildDetailPrompt(work, volumeIndex, userCommand, prevResult) {
  ...
  prompt += '### 【第N章】章节标题\n';
  prompt += '**情绪基调**：本章的整体情绪走向（如：绝望→惊险→冷峻中的希望）\n';
  prompt += '**爽点类型**：本章的核心爽点（如：金手指展示+武力震撼）\n';
  prompt += '**时间**：具体时间点（如：光和六年四月上旬，暮色至深夜）\n';
  prompt += '**地点**：本章主要发生的地点（如：姑臧城外破土屋、门槛、土坡）\n';
  prompt += '**人物**：本章出场的主要角色（如：刘璟、张伯、韩三、打手六人）\n';
  prompt += '**硬节点**：\n';
  prompt += '1. 第一个具体剧情节点\n';
  prompt += '2. 第二个具体剧情节点\n';
  prompt += '...\n';
  prompt += '20. 第二十个具体剧情节点\n';
  prompt += '**【数字面板】** 人数：X | 存粮：X | 刀：X | 棍：X | 马：X | 钱：X\n';
  ...
  prompt += '5. 数字面板必须包含实时资源统计（人数、存粮、武器、马匹、金钱等）\n';
