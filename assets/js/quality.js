...
(function () {
  ...
  function evaluateWorld(content) {
    ...
    // D1: 设定完整度（10分）— v59: 三级命中率
    var d1 = { name: '设定完整度', score: 6, max: 10, weight: 0.18, issues: [], strengths: [] };
    var w1a = /(力量体系|修炼体系|魔法体系|科技体系|灵气|内力|法力|魔力|基因|纳米|量子)/.test(content);
    var w1b = /(势力|门派|家族|国家|组织|联盟|帝国|王国|公会|帮派|宗门|世家)/.test(content);
    var w1c = /(代价|限制|规则|约束|瓶颈|天花板|副作用|反噬|代价|缺陷|弱点|短板|克星|天敌)/.test(content);
    var w1score = (w1a ? 1 : 0) + (w1b ? 1 : 0) + (w1c ? 1 : 0);
    if (w1score >= 3) { d1.score = 9; d1.strengths.push('设定完整（力量体系+势力+代价限制）'); }
    else if (w1score >= 2) { d1.score = 7; d1.strengths.push('有核心设定结构'); }
    else if (w1score >= 1) { d1.score = 5; d1.issues.push('设定偏少，仅' + w1score + '/3项'); }
    else { d1.score = 3; d1.issues.push('缺少力量体系/势力/代价限制'); }
    dims.push(d1);

    // D2: 独特创新（10分）— v59: 净得分映射
    var d2 = { name: '独特创新', score: 5, max: 10, weight: 0.18, issues: [], strengths: [] };
    var d2_penalty = /(四大神兽|五行|元素|斗气|魔法|魔力|灵气|内功|金木水火土|风火水土)/.test(content) ? 2 : 0;
    var d2_bonus = countMatches(content, /(独特|特殊|变异|扭曲|污染|异化|融合|嫁接|改造|变异|诡异|怪诞|奇异|扭曲|禁术|邪术|秘术|传承|血脉|天赋|觉醒|进化|蜕变|另类|颠覆|混沌|腐蚀|寄生|共生|同化|感染|侵入|侵蚀|吞噬|蔓延|扩散|增殖|繁殖|滋长|滋生|孕育)/g);
    var d2_net = 5 - d2_penalty + Math.min(d2_bonus, 5);
    d2.score = Math.max(2, Math.min(10, d2_net));
    if (d2_bonus >= 3) d2.strengths.push('独特设定丰富(' + d2_bonus + '处)');
    if (d2_penalty > 0) d2.issues.push('使用了传统元素，建议增加独特变体');
    if (d2_bonus === 0 && d2_penalty === 0) d2.issues.push('缺少独特设定元素');
    dims.push(d2);

    // D3: 冲突张力（10分）— v59: 三级评分
    var d3 = { name: '冲突张力', score: 5, max: 10, weight: 0.18, issues: [], strengths: [] };
    var w3a = /(矛盾|冲突|对立|战争|争斗|竞争|掠夺|资源|争夺|霸权|统治|反抗|压迫|不公|阶级|仇恨|世仇|恩怨|宿敌|对手|死敌|敌对|威胁|危机|末日|灾难|浩劫)/.test(content);
    var w3b = /(平衡|共存|制衡|牵制|博弈|僵局|均势|妥协|协议|盟约|停战|和平|共存)/.test(content);
    if (w3a && w3b) { d3.score = 8; d3.strengths.push('冲突源+势力平衡兼备'); }
    else if (w3a) { d3.score = 6; d3.strengths.push('有冲突源'); }
    else { d3.score = 3; d3.issues.push('缺少冲突源和势力矛盾'); }
    dims.push(d3);

    // D4: 社会结构（10分）— v59: 三级评分
    var d4 = { name: '社会结构', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var w4a = /(阶层|等级|阶级|贵族|平民|奴隶|自由民|身份|地位|出身|血统|天赋|资质|品级|阶级固化|上升通道)/.test(content);
    var w4b = /(经济|货币|贸易|市场|资源|矿产|粮食|特产|商路|交易|流通|财富|贫富|税收|赋税)/.test(content);
    if (w4a && w4b) { d4.score = 8; d4.strengths.push('阶层+经济体系完整'); }
    else if (w4a || w4b) { d4.score = 6; d4.strengths.push('有部分社会结构设计'); }
    else { d4.score = 3; d4.issues.push('缺少社会结构和经济体系'); }
    dims.push(d4);

    // D5: 叙事潜力（10分）— v59: 三级评分
    var d5 = { name: '叙事潜力', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var w5a = /(秘密|谜团|传说|预言|遗迹|宝藏|秘境|禁地|封印|诅咒|上古|远古|失落|毁灭|重启|轮回|转世|宿命|命运|使命|天选|预言者|救世主|终焉|末法|末路|新生|重铸|重建)/.test(content);
    if (w5a && len >= 800) { d5.score = 8; d5.strengths.push('叙事驱动元素丰富'); }
    else if (w5a) { d5.score = 6; d5.strengths.push('有叙事驱动元素'); }
    else { d5.score = 3; d5.issues.push('缺少故事驱动元素（秘密/预言/遗迹等）'); }
    dims.push(d5);
  ...
  function evaluateChars(content) {
    ...
    // D1: 人物立体度（10分）— v59: 三级命中率
    var d1 = { name: '人物立体度', score: 6, max: 10, weight: 0.18, issues: [], strengths: [] };
    var c1a = /(动机|欲望|想要|追求|目标|梦想|野心|执念|信念|信仰|原则|底线|软肋|弱点|缺陷|短板|恐惧|害怕|逃避|隐藏|隐瞒|秘密|过去|往事|创伤|阴影|童年|来历)/.test(content);
    var c1b = /(成长|变化|改变|蜕变|觉醒|领悟|转折|醒悟|看透|放下|拿起|坚持|放弃|选择|代价|牺牲|换取|交换|交易|背叛|忠诚|信任|怀疑|试探|考验|抉择)/.test(content);
    if (c1a && c1b) { d1.score = 8; d1.strengths.push('动机/弱点+角色弧光兼备'); }
    else if (c1a || c1b) { d1.score = 6; d1.strengths.push('有部分人物深度设计'); }
    else { d1.score = 3; d1.issues.push('人物缺少动机/弱点/弧光，趋于扁平'); }
    dims.push(d1);

    // D2: 关系网络（10分）— v59: 三级评分
    var d2 = { name: '关系网络', score: 5, max: 10, weight: 0.18, issues: [], strengths: [] };
    var c2a = (content.match(/[\u4e00-\u9fa5]{2,4}(?=\s*[（(]|：|:|\n|$)/g) || []).length >= 3;
    var c2b = /(朋友|敌人|对手|盟友|导师|徒弟|家人|爱人|恋人|仇人|情敌|竞争者|同事|上下级|主仆|搭档|同伴|伙伴|知音|知己|恩人|仇家)/.test(content);
    if (c2a && c2b) { d2.score = 8; d2.strengths.push('人物识别正常+关系描述完整'); }
    else if (c2a || c2b) { d2.score = 6; d2.strengths.push('有基本关系网络'); }
    else { d2.score = 3; d2.issues.push('人物关系网络不明显'); }
    dims.push(d2);

    // D3: 反派/对抗力（10分）— v59: 三级评分
    var d3 = { name: '反派/对抗力', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var c3Count = countMatches(content, /(反派|对手|敌人|敌对|仇敌|死敌|威胁|幕后|黑手|操控|阴谋|boss|BOSS|魔头|魔王|枭雄|霸主|强者|高手|劲敌|怪物|异形|邪神|恶魔|妖|魔|鬼|怪|邪|恶|暗|黑)/g);
    if (c3Count >= 3) { d3.score = 8; d3.strengths.push('反派/对抗力丰富(' + c3Count + '处)'); }
    else if (c3Count >= 1) { d3.score = 6; d3.strengths.push('有反派/对抗力'); }
    else { d3.score = 3; d3.issues.push('缺少明确的反派/对抗力量'); }
    dims.push(d3);

    // D4: 角色差异化（10分）— v59: 连续密度评分
    ...
    var diffDensity = len >= 500 ? diffCount / (len / 500) : 0;
    if (diffDensity >= 1.5) { d4.score = 8; d4.strengths.push('性格差异化描述丰富(' + diffCount + '处)'); }
    else if (diffDensity >= 0.5) { d4.score = 6; }
    else { d4.score = 3; d4.issues.push('角色之间缺乏差异化描述'); }
    dims.push(d4);

    // D5: 情感共鸣（10分）— v59: 三级评分
    var d5 = { name: '情感共鸣', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var c5Count = countMatches(content, /(共鸣|代入|共情|心疼|感动|愤怒|同情|厌恶|喜欢|憎恨|敬佩|鄙视|羡慕|嫉妒|恨|爱|怜|惜|叹|哀|悲|喜|怒|惊|恐|忧|愁|痛|苦|煎|熬|挣扎|无助|孤独|温暖|希望|力量|勇气|选择|放弃|坚持)/g);
    if (c5Count >= 3) { d5.score = 8; d5.strengths.push('情感共鸣设计丰富(' + c5Count + '处)'); }
    else if (c5Count >= 1) { d5.score = 6; d5.strengths.push('有情感共鸣元素'); }
    else { d5.score = 3; d5.issues.push('缺少情感共鸣设计'); }
    dims.push(d5);
  ...
  function evaluateOutline(content) {
    ...
    // D3: 伏笔设计（10分）— v59: 三级评分
    var d3 = { name: '伏笔设计', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var o3Count = countMatches(content, /(伏笔|暗示|铺垫|隐藏|暗线|伏线|回收|呼应|照应|对应|首尾呼应|前后照应|埋线|揭线|线索|谜底|答案|揭示|真相|才发现|原来|竟然|居然|其实|就是|就是他就是|凶手是|幕后|操控|真面目|身份|身世|来历|渊源|因果|报应|宿命|天意|人为|巧合|必然|偶然)/g);
    if (o3Count >= 3) { d3.score = 8; d3.strengths.push('伏笔/暗线设计丰富(' + o3Count + '处)'); }
    else if (o3Count >= 1) { d3.score = 6; d3.strengths.push('有伏笔/暗线设计'); }
    else { d3.score = 3; d3.issues.push('缺少伏笔/暗线设计'); }
    dims.push(d3);

    // D4: 主线清晰（10分）— v59: 三级评分
    var d4 = { name: '主线清晰', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var o4Count = countMatches(content, /(主线|核心|目标|任务|使命|冒险|旅程|征途|探索|追查|复仇|寻找|守护|拯救|建立|推翻|恢复|改变|逃离|归来|回家|登顶|称霸|统一|分裂|独立|自由|解放|救赎|赎罪|证明|超越|突破|成为|战胜|击败|消灭|收服|收编|联合|结盟|对抗|抵抗|反攻|反击|逆袭|翻盘|崛起|陨落|升迁|贬谪|流放|回归)/g);
    if (o4Count >= 3) { d4.score = 8; d4.strengths.push('主线清晰(' + o4Count + '处)'); }
    else if (o4Count >= 1) { d4.score = 6; d4.strengths.push('有主线设计'); }
    else { d4.score = 3; d4.issues.push('主线不够明确'); }
    dims.push(d4);

    // D5: 冲突递进（10分）— v59: 三级评分
    var d5 = { name: '冲突递进', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var o5Count = countMatches(content, /(升级|加剧|恶化|扩大|蔓延|扩散|失控|爆发|决裂|底线|摊牌|最后通牒|背水一战|殊死|你死我活|鱼死网破|不是你死就是我亡|势不两立|不共戴天|血海深仇|深仇大恨|新仇旧恨|恩恩怨怨|因果循环|冤冤相报|冤家路窄|狭路相逢|势均力敌|难分胜负|不分伯仲|旗鼓相当|针锋相对|寸步不让|步步紧逼|得寸进尺|赶尽杀绝|斩草除根|不留后患|斩尽杀绝|一网打尽|一网成擒|一劳永逸|永绝后患)/g);
    if (o5Count >= 3) { d5.score = 8; d5.strengths.push('冲突递进设计丰富(' + o5Count + '处)'); }
    else if (o5Count >= 1) { d5.score = 6; d5.strengths.push('有冲突递进设计'); }
  ...
  function evaluateDetail(content) {
    ...
    // D3: 信息释放（10分）— v59: 释放量 vs 堆砌量双因子
    var d3 = { name: '信息释放', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var d3RelCount = countMatches(content, /(揭示|揭露|透露|暴露|展现|展示|呈现|浮现|浮出水面|水落石出|真相大白|层层推进|逐步|渐进|依次|先后|先是|接着|然后|再|又|还|更|更加|尤为|尤其|特别|格外|异常|异乎寻常|出奇|与众不同|独具一格|别具一格|独一无二|史无前例|前所未有|前所未闻|闻所未闻|见所未见|想象不到|意料之外|出乎意料|始料未及|猝不及防|措手不及)/g);
    var d3DumpCount = countMatches(content, /(堆砌|集中|一次性|突然|猛然|骤然|陡然|蓦然|倏忽|刹那|瞬间|片刻|霎时|转瞬|转眼|须臾|弹指|一瞬|一刹那|一瞬间|一眨眼|一刻|片刻间|眨眼间|转瞬间|转瞬之间|一瞬之间|呼吸之间|一呼一吸)/g);
    if (d3RelCount >= 3) { d3.score = 7; d3.strengths.push('信息释放有序'); }
    else if (d3RelCount >= 1) { d3.score = 6; d3.strengths.push('有信息释放节奏'); }
    if (d3DumpCount >= 2) { d3.score = Math.max(3, d3.score - 1); d3.issues.push('信息释放可能有堆砌风险'); }
    dims.push(d3);

    // D4: 节奏控制（10分）— v59: 三级评分
    var d4 = { name: '节奏控制', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var d4RhythmCount = countMatches(content, /(高潮|低潮|低谷|缓冲|过渡|铺垫|爆发|蓄力|高潮迭起|一波三折|跌宕起伏|张弛有度|松紧有度|紧松交替|一张一弛|一紧一松|一松一紧|时紧时松|忽紧忽松|急缓相间|快慢结合|虚实|动静|快慢|急缓|轻重|缓急|轻重缓急|详略|详略得当|疏密|有疏有密)/g);
    if (d4RhythmCount >= 2) { d4.score = 8; d4.strengths.push('节奏变化丰富'); }
    else if (d4RhythmCount >= 1) { d4.score = 7; d4.strengths.push('有节奏变化'); }
    if (chapterCount > 0) {
    ...
    // D5: 爽点分布（10分）— v59: 密度映射（修复双条件覆盖冲突）
    var d5 = { name: '爽点分布', score: 5, max: 10, weight: 0.16, issues: [], strengths: [] };
    var coolCount = countMatches(content, /(爽点|打脸|逆袭|翻盘|碾压|秒杀|装逼|扮猪吃虎|越级|越阶|突破|觉醒|传承|机缘|奇遇|宝藏|夺宝|比赛|试炼|选拔|大比|考核|拍卖|赌约|对决|决战|复仇|报仇|雪恨|翻转|打脸啪啪)/g);
    if (chapterCount > 0 && chapterCount <= 1000) {
      var coolDensity = coolCount / chapterCount;
      if (coolDensity >= 0.5) { d5.score = 8; d5.strengths.push('爽点密度充足(' + Math.round(coolDensity * 100) + '%)'); }
      else if (coolDensity >= 0.2) { d5.score = 6; d5.strengths.push('有爽点分布'); }
      else { d5.score = 3; d5.issues.push('爽点分布不足(每章' + Math.round(coolDensity * 100) + '%)'); }
  ...
  // ============ 架构模块质量驱动生成器（P0: QualityDriver）============
  // 将评分结果转换为针对性改进 prompt，驱动 AI 定向修复低分维度
  function buildModuleEnhancePrompt(module, report, currentContent, targetGoal) {
    var lowDims = report.dimensions.filter(function(d) { return d.score < 7; });
    if (lowDims.length === 0) return '';

    var moduleNames = { world: '世界观', chars: '人设', outline: '大纲', detail: '细纲', chapter: '正文' };
    var moduleName = moduleNames[module] || module;
    var prompt = '【★ ' + moduleName + ' 质量加强 — 仅修复低分维度，其他部分保持不动】\n';
    prompt += '⚠️ 核心要求：本次生成是在已有内容上做"精准修补"，不是重新生成。\n';
    prompt += '⚠️ 原始用户核心诉求是生成基础，绝对不可改变或重新解读。\n';
    prompt += '⚠️ 只修改下面列出的低分维度，其他部分必须保持原样。\n\n';

    lowDims.forEach(function(dim) {
      var improvement = getDimensionImprovement(module, dim.name, dim.issues);
      if (improvement) {
        prompt += '【↓ ' + dim.name + '（' + dim.score + '/10 分）需要加强】\n' + improvement + '\n\n';
      }
    });

    // 加入字数要求
    if (currentContent && currentContent.trim()) {
      prompt += '【字数要求】当前内容约 ' + currentContent.trim().length + ' 字。加强后总字数不得低于当前字数。\n';
      prompt += '每删除一个不符合要求的内容，必须用同等篇幅的合规内容填补。整体字数不得减少。\n\n';
    }
  ...
  // 各模块各维度的改进指南库
  function getDimensionImprovement(module, dimName, issues) {
    var guides = {
      world: {
        '设定完整度': '力量体系深度不足。请按以下结构重写力量体系部分：\n' +
          '①等级名称（至少5级，含名称+实力描述）；\n' +
          '②每个等级的标志性能力（不是泛泛说"强"，要具体）；\n' +
          '③等级晋升代价（升级需要什么？为什么有人卡在某个等级？）；\n' +
          '④势力与等级的关系（哪些势力垄断哪些等级的资源？）\n' +
          '⑤等级压制与越级机制（高等级对低等级的优势是什么？低等级靠什么弥补？）',
        '独特创新': '世界观缺少独特设定。建议在当前世界观基础上增加：\n' +
          '①至少1个"区别于同类作品"的核心设定（如：力量来源不是修炼而是交易；升级需要付出真实代价）；\n' +
          '②传统元素的变体（如有门派，要让门派有独特的晋升逻辑和内部矛盾；如有魔法，要有使用限制和反噬）；\n' +
          '③一个让读者觉得"这个设定有意思"的记忆点',
        '冲突张力': '世界观缺少核心矛盾。请为世界观设计：\n' +
          '①至少2股势力之间的根本冲突（不是"正邪对立"，而是资源/信仰/历史仇恨层面的真实矛盾）；\n' +
          '②冲突如何影响普通人的生活（老百姓站在哪边？）；\n' +
          '③主角身处冲突中的位置（被迫选择/主动站队/试图中立但被卷入）',
        '社会结构': '社会结构设计薄弱。请增加：\n' +
          '①社会的阶层划分（贵族/平民/奴隶，或独特的阶层体系）；\n' +
          '②经济基础（货币/贸易/资源分布）；\n' +
          '③上升通道（底层如何改变命运？阶层固化的程度如何？）；\n' +
          '④社会矛盾（阶层间的张力如何酝酿冲突？）',
        '叙事潜力': '世界缺少故事驱动的核心引擎。请增加：\n' +
          '①至少1个贯穿全书的"谜团"（上古遗迹/预言/失落的历史/神秘的诅咒）；\n' +
          '②这个谜团如何与主角的命运挂钩（为什么只有主角能解开？）；\n' +
          '③谜团分阶段揭示的节奏设计（前期线索→中期转折→终极真相）'
      },
      chars: {
        '人物立体度': '角色缺少"不可替代性"。请为每个核心角色增加：\n' +
          '①一个只有他能做出的选择（别人面对同样情况会做不同选择）；\n' +
          '②一个致命的性格缺陷（不是泛泛的"优柔寡断"，要具体到行为模式）；\n' +
          '③一个内心深处的恐惧（影响决策但主角自己未必意识到）；\n' +
          '④一段过去创伤（解释他为什么会形成现在的性格）',
        '关系网络': '人物关系网不够复杂。请在现有关系基础上增加：\n' +
          '①每对关系至少有一种"张力来源"（利益冲突/情感纠葛/地位差距/秘密隐瞒）；\n' +
          '②核心角色身边至少有3个"有态度"的关系对象（不只对主角好，他们有各自的目的）；\n' +
          '③反派与主角之间除了对立之外，应有某种"镜像关系"或"共同点"',
        '反派/对抗力': '反派/对抗力量刻画不足。请增强：\n' +
          '①反派必须有"自洽的逻辑"——他认为自己在做正确的事，不是纯粹的坏；\n' +
          '②反派的实力必须有阶段性压制——主角不能一开始就碾压；\n' +
          '③至少有一个"让读者也动摇"的反派观点（读者会觉得"他说的好像也有道理"）',
        '角色差异化': '角色之间缺乏鲜明区分。请为每个核心角色赋予：\n' +
          '①独特的说话风格（语速/用词习惯/口头禅/语气特色）；\n' +
          '②面对压力时的典型反应（有人暴怒/有人沉默/有人冷笑/有人转移话题）；\n' +
          '③2-3个标志性行为模式（如紧张时摸耳朵、生气时压低声音、思考时转笔）；\n' +
          '④不低于1个"与主角截然相反"的核心特质',
        '情感共鸣': '角色缺乏让读者共情的设计。请增加：\n' +
          '①主角至少有一个"普通人的软肋"（对家人/朋友/某个信念的执着）；\n' +
          '②至少有一段"读者会心疼主角"的设计（努力被误解/牺牲不被理解/独自承受压力）；\n' +
          '③配角至少有一个"高光时刻"（为保护主角牺牲/在关键时刻说出金句/展示不为人知的一面）'
      },
      outline: {
        '结构清晰': '大纲结构不清晰。请按以下结构重新组织：\n' +
          '①全书分为若干卷，每卷有独立的核心冲突和情绪目标；\n' +
          '②每卷标注：本卷核心冲突、本卷爽点、本卷高潮位置、本卷结尾钩子；\n' +
          '③卷与卷之间必须有递进关系（难度升级/危机扩大/关系复杂化）；\n' +
          '④每卷标注字数范围和卷内章节数量（建议每卷15-30章）',
        '节奏把控': '大纲缺少清晰的节奏安排。请为全书设计明确的节奏节点：\n' +
          '①第1-5章：建立核心冲突+展示金手指（黄金三章法则：第1章危机，第2章金手指，第3章打脸）；\n' +
          '②第6-30章：冲突递进+每10章一次重大反转；\n' +
          '③高潮卷：蓄力3-5章后爆发（不得连续高潮，要有低谷缓冲）；\n' +
          '④结尾卷：收束伏笔+情感高潮+结局\n' +
          '⑤每章标注"情绪目标"（如：压抑→释放、紧张→惊喜、疑惑→震撼）',
        '伏笔设计': '伏笔体系不足。请建立三级伏笔体系：\n' +
          '①长期伏笔（全书级）：开头即埋下，结局才回收（主角身世之谜/世界真相/宿敌的真正身份）；\n' +
          '②中期伏笔（每卷级）：卷开头埋下，卷结尾回收（本卷宝物/本卷反派/本卷秘密）；\n' +
          '③短期伏笔（章节级）：每章结尾制造悬念，下一章或下几章回应；\n' +
          '④特别要求：每卷至少设计2个"伏笔回收"的时刻（读者会感到"原来如此"）',
        '主线清晰': '主线不够明确。请重新梳理主线：\n' +
          '①主角的终极目标是什么？（不是一个笼统的方向，是一个可量化的终点）；\n' +
          '②达成这个目标需要哪几个关键步骤？（每步对应一卷或几卷）；\n' +
          '③阻碍主角达成目标的根本力量是什么？（不是"反派阻挡"，而是更底层的问题）；\n' +
          '④主线之外，为每条重要副线设计1-2个"高光交叉点"（副线与主线交汇的时刻）',
        '冲突递进': '冲突缺少递进升级。请设计冲突升级阶梯：\n' +
          '①第一阶段：个人冲突（主角与身边的人/小势力）；\n' +
          '②第二阶段：团体冲突（主角的团体与敌方团体）；\n' +
          '③第三阶段：势力冲突（主角所在的势力与敌对势力）；\n' +
          '④第四阶段：世界观级冲突（影响整个世界格局的对抗）；\n' +
          '⑤每个阶段升级必须有"导火索"事件（不是自然升级，是某个事件催化了升级）',
        '商业价值': '爽点/商业价值设计不足。请加强：\n' +
          '①每卷至少设计了几个"名场面"（打脸/逆袭/扮猪吃虎/深情告白/绝地反击）；\n' +
          '②前10章爽点密度：至少每3章一个爽点，前3章必须有"打脸"或"逆袭"名场面；\n' +
          '③爽点类型多样化（打脸型/收获型/成长型/情感型，不要只有一种）；\n' +
          '④每章标注"爽点类型"，确保爽点分布均匀'
      },
      detail: {
        '章节连贯': '章节间连贯性不足。请检查：\n' +
          '①每章开头的第一个事件是否能追溯到前一章结尾的因果链（不是因为时间到了"第二天"，而是因为上一章结尾的X事件导致了本章开头的Y事件）；\n' +
          '②每章格式统一使用「■ 第N章《标题》| 场景：xxx | 人物：xxx | 剧情节点：xxx | 爆点/悬念钩子：xxx」格式；\n' +
          '③章节编号必须连续，不得跳号',
        '黄金三章': '前3章不符合黄金法则。请按以下重写前3章细纲：\n' +
          '第1章（冲突建立）：主角必须在300字内遭遇"生存危机"（不是困难，是生死级别的）；\n' +
          '  ①危机展示→②主角的困境（无能力/被压迫/走投无路）→③一线生机（金手指觉醒/意外援手/下定决心）；\n' +
          '第2章（金手指展现）：主角在"看似绝境"时第一次使用金手指/能力；\n' +
          '  ①利用新能力应对危机→②效果远超预期→③引起注意（有人开始关注主角）；\n' +
          '第3章（首次打脸）：必须有第一次"名场面"（让读者立刻体验爽感）；\n' +
          '  ①冲突升级→②敌强我弱的态势→③主角用能力/智慧完美反击→④配角的反应（震惊/羡慕/嫉妒）',
        '钩子密度': '每章结尾钩子密度不足。请逐章检查并修改：\n' +
          '①本章最后一个事件必须是"未完成状态"（战斗未分胜负/秘密未完全揭露/危机未解除/决策未公开）；\n' +
          '②每章结尾必须有一个"意外信息"（新人物登场/关系突变/计划被打乱/意外发现）；\n' +
          '③如果某章以"解决"结尾，下一章开头必须立刻制造新的未完成状态；\n' +
          '④钩子类型应交替使用：危机型（危险逼近）→信息型（秘密揭露）→关系型（关系突变）→决策型（重要选择）',
        '信息释放': '信息释放节奏不均衡。请调整：\n' +
          '①每章释放多少新信息要有明确节奏——不要第三章就把世界观一切说完，也不要50章了还什么都没讲；\n' +
          '②核心秘密分阶段释放：前期给线索（挑动好奇心）→中期给部分真相（推动剧情）→后期给完整真相（迎来高潮）；\n' +
          '③避免"信息集中堆砌"——不要在一章内展示所有设定，分散到多章通过不同事件自然揭示',
        '节奏控制': '每章节奏设计不均。请为每章设计"起承转合"情绪曲线：\n' +
          '①起（10-20%）：建立场景/情绪基础；\n' +
          '②承（30-50%）：推进冲突/展示困难；\n' +
          '③转（15-25%）：高潮发生在章节70-85%位置——冲突爆发/真相揭露/决策时刻；\n' +
          '④合（10-15%）：结尾钩子——意外/悬念/未完待续；\n' +
          '⑤整章至少设计1处"意外打断"（谈话被中断/计划被打乱/平静被打破）',
        '爽点分布': '爽点分布不均匀。请优化：\n' +
          '①前10章爽点密度不低于每3章1个；\n' +
          '②整卷至少30%的章节包含爽点/名场面；\n' +
          '③爽点类型多样化：实力碾压型、智商在线型、情感爆发型、收获惊喜型、扮猪吃虎型；\n' +
          '④不要连续3章没有爽点',
        '可执行性': '细纲不够详实。请扩展每章细纲：\n' +
          '①每章至少包含3个明确的故事节点（开端→发展→转折/钩子）；\n' +
          '②每个节点标注"情绪目的"（本步想让读者感觉到什么？紧张/愤怒/惊喜/感动）；\n' +
          '③明确每章的核心冲突和角色目标（主角想做什么？谁在阻止他？）'
      }
    };

    // 正文（chapter）通用改进指南
    var chapterGuides = {
      '开篇吸引力': '前300字必须立即建立冲突/危机/悬念。砍掉环境描写和背景介绍的开头，直接从动作/事件切入。',
      '章尾钩子': '章尾必须停在"未完成状态"：战斗未结束/秘密未揭晓/关键信息刚到嘴边/意外突然发生。',
      '对话质量': '给每个角色独特的说话方式。对话中穿插动作和微表情。增加潜台词（话里有话）。',
      '动作描写': '增加具体、独特的身体动作：细小的手势、不自觉的小动作、反常的沉默。',
      '感官描写': '每500字至少1处非视觉感官：声音/气味/触感/温度。',
      '情绪张力': '删除所有"XX很XX"的直白情绪词，用微动作+环境暗示替代。',
      '节奏控制': '紧张处短句冲（5-10字），舒缓处长句铺（20-30字）。段落长短交替。',
      '文笔水平': '替换所有AI模板化表达（"众人震惊""空气凝固""瞳孔一缩"等），用具体独特的描写替代。',
      '信息密度': '每500字至少推进1个新信息点。删除注水段落（重复已知信息/无意义闲聊）。',
      '原创性': '减少套路化表达。每个角色有自己的标志性动作和说话方式。',
      '修辞手法': '关键场景加入比喻强化画面感，情绪高潮用排比冲击力，环境用通感。',
      '句式多样性': '紧张场景至少50%短句（≤10字）；叙事场景允许长句但不超过30%；对话中穿插反问感叹。',
      '用词精准度': '用精准动词替代通用词（说/看/走/拿）。删除"很/非常/极其"。一个精准词比三个堆砌有力。',
      '描写细腻度': '每章至少3处环境描写+3处外貌神态细节。环境与人物互衬。'
    };

    var moduleGuides = guides[module];
    if (moduleGuides && moduleGuides[dimName]) return moduleGuides[dimName].improvement;

    // 正文维度回退
    if (module === 'chapter' || module === 'text') {
      var chGuide = chapterGuides[dimName];
      if (chGuide) return chGuide;
    }

    // 通用回退
    var issuesText = (issues && issues.length) ? issues.slice(0, 2).join('；') : '需提升';
    return '改进方向：' + issuesText + '。请针对此维度进行加强，保持其他部分不变。\n提供具体修改建议：哪些文字需要替换/修改/删除/补充，以及具体改法。';
  }
  ...
  window.QualityEngine = {
    ...
    // P0: 架构模块质量驱动生成器
    buildModuleEnhancePrompt: buildModuleEnhancePrompt,
    getDimensionImprovement: getDimensionImprovement,
    // 兼容旧 API
    score: makeReport,
    attach: attach,
    lastHints: lastHints,
